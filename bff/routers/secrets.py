...
class SecretStore(BaseModel):
    ...
    scope:       ScopeT
    workspaceId: Optional[str] = None
    rawValue:    str           # never exposed in GET
    createdAt:   str
    updatedAt:   str
    createdBy:   str
    tags:        list[str] = []
...
def list_secrets(scope: Optional[str] = Query(None)):
    items = list(_STORE.values())
    if scope: items = [s for s in items if s.scope == scope]
...
def rotate_secret(secret_id: str, body: RotateRequest):
    ...
    updated = s.copy(update={'rawValue': body.newValue, 'updatedAt': _now()})
    _STORE[secret_id] = updated
    return _view(updated)

@router.delete('/{secret_id}')
def delete_secret(secret_id: str):
    if secret_id not in _STORE: raise HTTPException(404, 'Secret not found')
