...
async def try_model(primary: str, fallback: str = "vllm") -> str:
...
async def route_request(task_complexity: str, context_length: int) -> str:
    ...
    if context_length > DEVSTRAL_CTX_LIMIT:
        ...
        return await try_model(FAST_MODEL, fallback="vllm")
    if task_complexity == "agentic":
        return await try_model(PRIMARY_MODEL, fallback="vllm")
    return await try_model(FAST_MODEL, fallback="vllm")
