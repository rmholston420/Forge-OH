...
_KIND_TO_TYPE: dict[str, str] = {
    ...
    # Stage 6.1 — web-search consultation. Emitted by
    # `bff/services/search_events.py` when the `search_web` tool posts to
    # the bridge endpoint. Carries `query` (str), `result_count` (int),
    # `provenance` (str, e.g. "searxng:http://127.0.0.1:18888"), and
    # `latency_ms` (int). Rendered by EventCard as a compact marker.
...
def _web_search_summary(ev: dict[str, Any]) -> str:
    ...
    Falls back gracefully when any field is missing so a partial event
    never blanks the timeline row.
    """
    if ev.get("summary"):
        return str(ev["summary"])
    query = ev.get("query")
    ...
    return f"Web searched: {query_str} — {count_str}"
