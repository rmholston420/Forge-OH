"""openhands_tools_ext.memory.adapters.dozerdb.dozerdb_graph_backend — Real Bolt GraphBackend.

Ported verbatim from Kosmos `adapters.memory.dozerdb.dozerdb_graph_backend`
@ SHA c455165. See PORTING_LEDGER.md for provenance.

Wraps `neo4j.AsyncGraphDatabase` (works against DozerDB 5.26.x which is
Neo4j-Bolt-compatible). Satisfies the `GraphBackend` Protocol declared in
``openhands_tools_ext.memory.adapters.dozerdb.adapter``. All `neo4j`
imports are lazy so the fast unit tier does not require the driver on
``PYTHONPATH``.
...
class DozerDbGraphBackend:
    """Bolt-backed `GraphBackend` for the DozerDbMemoryAdapter.
    ...
    live DozerDB is required for the fast tier. Forge-OH points this at the
    shared Colossus DozerDB container (see composition.py + Forge-OH
    ADR-019 for the shared-instance option).
    """
